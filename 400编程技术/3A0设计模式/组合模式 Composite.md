...
*Add、Remove 函数放在基类还是放在派生类，是个问题*

**将对象组合成树形结构**，不同种类的数据继承同一个抽象基类并作为树的节点，不同种类的数据节点的接口相同。


**总结**

- Composite 模式采用树形结构来实现普遍存在的对象容器，从而将 “一对多” 的关系转化为 “一对一” 的关系，使得客户代码可以一致地（复用）处理对象和对象容器，无需关心处理的是单个的对象，还是组合的对象容器。 **-> 多态递归调用** **将对象组合成树形结构**
